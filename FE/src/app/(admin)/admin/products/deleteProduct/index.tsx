import { useDeleteProductMutation } from '@/app/utils/hooks/productsHooks'
...
export default function DeleteProduct({ showDelete, setShowDelete, refetch }: Props) {
  const fetchApi = useFetch()
  const { mutateAsync: deleteProduct } = useDeleteProductMutation(fetchApi)
  const handleClose = () =>
    setShowDelete({
      show: false,
      id: '',
  ...
  const handleDeleteProduct = async () => {
    const result = await deleteProduct(showDelete.id)
    if (result) {
      refetch()
      handleClose()
